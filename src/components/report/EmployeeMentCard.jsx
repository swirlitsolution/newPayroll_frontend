...
function EmployeeMentCard(props) {
  ...
  const generateEmployeementCardPDF = () => {
    ...
    props.employee?.forEach((employee, index) => {
      ...
      doc.setFont('times', 'bold');
      doc.text(`${props.company?.contractdata?.name || props.company?.contractEstablishment || 'N/A'}`, 650, positionY + 60, { align: 'left' });
      doc.setFont('times', 'normal');
      doc.text('Name & Address of Principal Employer', 650, positionY + 80, { align: 'left' });
      doc.setFont('times', 'bold');
      doc.text(`${props.company?.principledata?.name || props.company?.principleEmployer || 'N/A'}`, 650, positionY + 100, { align: 'left' });
