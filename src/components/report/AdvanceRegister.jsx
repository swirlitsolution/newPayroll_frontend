...
function AdvanceRegister(props) {
  ...
  const generateAdvancePDF = () => {
    ...
    doc.text(`${props.company?.contractdata?.name || props.company?.contractEstablishment || 'N/A'}`, 650, 50);

    doc.text('Name & Address of Principal Employer:', 650, 60);
    doc.text(`${props.company?.principledata?.name || props.company?.principleEmployer || 'N/A'}`, 650, 75);

    doc.text('Name & Location of Work:', 20, 90);
    doc.text(`${props.company?.worknaturedata?.name || props.company?.workNature || 'N/A'}`, 20, 100);
